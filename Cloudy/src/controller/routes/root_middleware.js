...
router.use(userSession, async (req, res, next) => {
    ...
    if (file_url === "/") {
        ...
        if (fileStream !== undefined) {
            ...
            const downloadStream = await mongoUtils.createDownloadStream(client, database, filename)

            // Gets the fileData for the file by filename
            let fileData = await mongoUtils.getFiles(client, database, {filename: filename});
